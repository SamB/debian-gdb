/* CYGNUS LOCAL tic54x/twall@tiac.net (entire file) */
...
#define COFF_ALIGN_IN_SECTION_HEADER 
/* requires a power-of-two argument */
#define COFF_ENCODE_ALIGNMENT(X) (((unsigned)(X)&0xF)<<8)
...
(COFF2_P(ABFD) ? bfd_h_put_16 (ABFD,VAL,PTR) : bfd_h_put_8 (ABFD,VAL,(PTR)-7))
...
((NAME[0] == '$' && isdigit(NAME[1]) && NAME[2] == '\0') \
 || NAME[strlen(NAME)-1] == '?')
...
#define R_RELBYTE 017               /* 8-bit direct reference to symbol addr */
#define R_REL13   0x002A            /* 13-bit direct reference (???) */
#define R_RELWORD 020               /* 16-bit direct reference */
